...
const sendOtp = async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.error('❌ User save failed:', saveErr.message);
      return res.status(500).json({ success: false, message: 'Failed to save OTP. Please try again.' });
    }

    // Log the event (non-blocking — never crash login because of logging)  // <--- This comment correctly states the intention
    try {
    ...
    try {
      ...
      emailSent = false;
      // Always expose OTP in terminal so dev/testing can proceed even if SMTP is broken
    ...
    res.json({
      ...
      otp: otp,
      emailSent,
      message: emailSent ? 'Code sent! Check your email inbox.' : 'Test mode: OTP auto-filled for testing.'
    });
  } catch (err) {
    next(err);
  }
