...
export default function LoginPage() {
  ...
  const inputRefs = useRef([]); // kept for backward compatibility if needed, but OtpInput manages its own
  ...
  const handleSendOtp = async (e) => {
    ...
    // Feature 11: Frontend validation
    const allowedDomains = ['@vdartinc.com', '@ndartinc.com'];
    const isVald = allowedDomains.some(d => email.toLowerCase().endsWith(d));
    ...
    try {
      ...
      setStep('otp');
      // Auto-fill OTP for testing when backend returns it
      if (res.data.otp) {
        const digits = res.data.otp.split('');
        setOtp(digits);
        // Auto-verify after short delay so state settles
        setTimeout(() => handleVerifyOtp(res.data.otp), 300);
      }
    } catch (err) {
      if (!err.response) {
  ...
  const handleResend = async () => {
    if (!onResend()) return; // Hook handles logic
  ...
  return (
    <div className="auth-page">
      {/* VDesk Premium Branding Panel */}
      <div className="auth-page__brand">
        <div className="auth-page__logo-fixed">
          <div className="flex-center" style={{ width: '42px', height: '42px', background: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
